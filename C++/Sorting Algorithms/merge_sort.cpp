...
public:
	...
	MergeSort(const vector<T>& elems) {
		items.assign(elems.begin(),elems.end());
		size = items.size();
	}

	void sort() {
		vector<T> vptr = mergeSort(items);
		items.assign(vptr.begin(),vptr.end());
	}
...
template<typename T>
vector<T> MergeSort<T>::mergeSort(vector<T>& arr) {
	if (arr.size() == 1) {
		return arr;
	}
	int n1 = arr.size() / 2; // n1 size of subarray 1
	int n2 = arr.size() - n1; // n2 size of subarray 2
	vector<T> arr1(n1); 
	vector<T> arr2(n2); 
	arr1.assign(arr.begin(),arr.begin()+n1); // copy n1 elements
	arr2.assign(arr.begin() + n1, arr.end()); // copy n2 elements

	return merge(mergeSort(arr1), mergeSort(arr2));
}

template <typename T>
vector<T> MergeSort<T>::merge(vector<T>& arr1, vector<T>& arr2) {
	int n1 = arr1.size();
	int n2 = arr2.size();
	int n = n1 + n2;
	vector<T> arr(n);
	...
	for (i = 0, j = 0, k = 0; i < n1 && j < n2; k++) {
		if (arr1[i] < arr2[j]) { //copy element from 1st array
			arr[k] = arr1[i++];
		}
		else { //copy element from 2nd array
			arr[k] = arr2[j++];
		}
	}
	while (i < n1) { // copy missing elements of sub-array 1
		arr[k++] = arr1[i++];
	}
	while (j < n2) { // copy missing elements of sub-array 2
		arr[k++] = arr2[j++];
	}
...
int main() {
	...
	MergeSort<int> list = MergeSort<int>(v);
	list.print();
	list.sort();
	cout << endl << endl;
	list.print();
